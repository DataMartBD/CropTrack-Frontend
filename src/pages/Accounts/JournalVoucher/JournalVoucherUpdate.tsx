...
export default function JournalVoucherUpdate() {
  ...
  useEffect(() => {
    const initData = async () => {
      setLoading(true);
      try {
        const [accountsData, projectsData, jvData] = await Promise.all([
        ...
        if (jvData) {
          setHeader({
            xdate: new Date(jvData.xdate),
            xlong: jvData.xlong || "",
            xyear: jvData.xyear,
            xper: jvData.xper,
            xref: jvData.xref || "",
            xstatusjv: jvData.xstatusjv || "Open",
            xtrngl: jvData.xtrngl || "JV--",
          });

          if (jvData.gldetails_header) {
            const mappedDetails = await Promise.all(
              jvData.gldetails_header.map(async (d: any) => {
                const account = accountsData?.find((a) => a.xacc === d.xacc);
                ...
                const detailRow: JournalDetail = {
                  ...
                  xlong: d.xlong || "",
                  xqty:
                    d.xqty !== null && d.xqty !== undefined
                      ? String(d.xqty)
                      : "",
                  debit: prime > 0 ? prime.toFixed(2) : "",
                  credit: prime < 0 ? Math.abs(prime).toFixed(2) : "",
                  accountName: account?.xdesc || "",
                  accountSource: account?.xaccsource || "",
                };

                if (
                  detailRow.accountSource === "Subaccount" ||
                  detailRow.accountSource === "Customer"
                ) {
                  await fetchSubAccounts(
                    detailRow.xacc,
                    detailRow.accountSource,
                  );
                }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await putData(`/accounts/Journal-voucher/${xvoucher}/`, payload);
      toast.success("Journal Voucher updated successfully");
      navigate("/accounts/journal-voucher");
    } catch (err) {
      console.error("Failed to update JV", err);
  ...
  return (
    <div className="pb-32">
      ...
      />
      <PageMeta
        title="Crop Track - Update Journal Voucher"
        description="Update Journal Voucher"
      />
      <PageBreadcrumb pageTitle="Update Journal Voucher" />
        ...
        <div className="flex w-full items-center justify-end gap-4 px-6 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>{submitting ? "Updating..." : "Update Voucher"}</span>
            </button>
          </div>
        </div>
      </div>
