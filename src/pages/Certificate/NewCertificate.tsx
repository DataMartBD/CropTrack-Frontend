...
  const [agents, setAgents] = useState<Agent[]>([]); // Added agent state
...
        // FIXED: Use recalculateAmounts here
        setFormData((prev) => {
          const updated = { ...prev, rent_per_sack: rentRate };
...
        // FIXED: Use recalculateAmounts here
        setFormData((prev) => {
          const updated = { ...prev, totalBags: bags };
          return recalculateAmounts(updated);
        });
      });
    }
...
  	        {agents.map((agent) => (
  	          <option key={agent.customer_code} value={agent.customer_code}>
  	            {agent.customer_code} - {agent.customer_name}
  	          </option>
  	        ))}
  	      </select>
  	    </div>
...
  	        disabled
  	        className="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-3 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-[#1Player] focus:outline-none disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:cursor-not-allowed"
  	      />
  	    </div>
  	  </div>
